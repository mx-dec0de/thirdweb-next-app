...
import { ThirdwebProvider, ConnectWallet, useAddress, useDisconnect, useChainId } from "@thirdweb-dev/react";
...
function WalletApp() {
    ...
    const fetchBalances = async () => {
      try {
        ...
        if (address && chainId && window.ethereum) {
          ...
          console.log("Chain ID in use:", chainId);
          setUserNetwork({ chainId });

          if (parseInt(chainId, 10) !== appConfig.network.chainId) {
            throw new Error(`Connected to wrong network: ${chainId}. Please connect to the ${appConfig.network.name} network.`);
          }
  ...
  const handleSendTransaction = async () => {
    try {
      ...
      await tx.wait();
      await fetchBalances(); // Refresh balances after sending
        ...
        <div className="content">
          ...
          {address && (
            <>
              {!loading ? (
                <>
                  <p>Your {userNetwork ? userNetwork.chainId : 'Network'} Address: {address}</p>
